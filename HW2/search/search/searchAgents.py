...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        action = []
        visited = []
        lastNode = {} # dictionary with key is the state and value is the tuples (previous node,action)
        direction = {}
        state = []
        node = []
        visited.append(startPosition)
        for n in problem.getSuccessors(startPosition):
		    lastNode[n[0]] = (startPosition,n[1])
		    queue.push(n)
        while not queue.isEmpty():
	        g = queue.pop()
	        visited.append(g[0])
	        if(problem.isGoalState(g[0])):
		        state = g[0]
		        break
	        else:
			    for m in problem.getSuccessors(g[0]):
			        if ((m[0] not in visited) and m[0] not in lastNode.keys()):
					    queue.push(m)
					    lastNode[m[0]] = (g[0],m[1])
        while (state != problem.getStartState()):
		    node.insert(0,state)
		    state = lastNode[state][0]
        for f in node:
	        action.append(lastNode[f][1])	
        return action
